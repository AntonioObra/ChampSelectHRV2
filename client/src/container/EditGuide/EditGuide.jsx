...
const EditGuide = () => {
  ...
  useEffect(() => {
    if (location.state) {
      ...
      setTitle(name);
      setPickedRole(role);
      setPickedChampion(champions[0]);
      setItemsSets(itemSets);
      setRuneSets(runeSets);
      setSynergieChampions(synergieChampions);
      setThreatChampions(threatChampions);
      setSpellsSets(spellsSets);
      setAbilitySets(abilitySets);
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(
      updateGuide(
        {
          id: location.state.guide._id,
          guide: {
            champion: pickedChampion.name,
            champions: [pickedChampion],
            name: title,
            role: pickedRole,
            itemSets: itemsSets,
...
export default ApperWrap(
  MotionWrap(EditGuide, "app__newguides"),
  "newguide",
  "app__whitebg"
);
